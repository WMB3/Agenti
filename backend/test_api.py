...
async def test_analyze_vehicle_no_client(test_app):
    ...
    # gemini_client is initially None in main.py
    async with AsyncClient(transport=ASGITransport(app=test_app), base_url="http://test") as ac:
...
async def test_analyze_vehicle_success(mock_gemini, test_app):
    ...
    # Set up the mock to return a controlled CarEvaluation object
    mock_analyze = AsyncMock(return_value=CarEvaluation(evaluation="Great condition", score=95))
    mock_gemini.analyze = mock_analyze
