...
@pytest.mark.asyncio
@patch('main.gemini_client', create=True)
async def test_analyze_vehicle_missing_payload(mock_gemini, test_app):
...
async def test_analyze_vehicle_gemini_error(mock_gemini, test_app):
    ...
    assert response.status_code == 500
    # You might also want to assert the detail message here if it's customized in the actual implementation
